...
const apiResource = "https://api.staging.company.example/orders/deploy"
...
func TestIssueConsumeIsExactAndOneUse(t *testing.T) {
	...
	if issued.GrantID != claims.GrantID || claims.MaxUses != 1 || claims.Audience != apiResource || claims.Resource != apiResource {
		t.Fatalf("unexpected claims: %+v", claims)
	}
	consumption := agentgrant.ConsumeRequest{Token: issued.Token, Resource: apiResource, Operation: operation}
...
func TestWrongPEPAudienceCannotConsumeOrBurnGrant(t *testing.T) {
	...
	consumption := agentgrant.ConsumeRequest{Token: issued.Token, Resource: apiResource, Operation: operation}
	if _, _, err := service.ConsumeForAudiences(consumption, bundle, now.Add(time.Second), []string{"https://bap-mcp-pep.company.example/mcp"}); err == nil {
		t.Fatal("MCP PEP audience consumed an API PEP grant")
	}
	if response, _, err := service.ConsumeForAudiences(consumption, bundle, now.Add(2*time.Second), []string{apiResource}); err != nil || !response.Consumed {
...
func TestIssueAndConsumeRejectChangedEvidence(t *testing.T) {
	...
	t.Run("changed operation", func(t *testing.T) {
		...
		if _, _, err := service.Consume(agentgrant.ConsumeRequest{Token: issued.Token, Resource: apiResource, Operation: changed}, bundle, now.Add(time.Second)); err == nil {
	...
	t.Run("changed policy", func(t *testing.T) {
		...
		if _, _, err := service.Consume(agentgrant.ConsumeRequest{Token: issued.Token, Resource: apiResource, Operation: operation}, changed, now.Add(time.Second)); err == nil {
			t.Fatal("stale policy-bound AgentGrant was accepted")
		}
	})
}

func TestResourceIndicatorIsMandatoryExactAndDoesNotBurnGrant(t *testing.T) {
	_, privateKey, _ := ed25519.GenerateKey(rand.Reader)
	now := time.Now().UTC().Truncate(time.Second)
	bundle := testBundle(now)
	operation := testOperation()
	...
	for name, resource := range map[string]string{
		"missing":        "",
		"malformed":      "orders-api",
		"query":          apiResource + "?tenant=other",
		"other resource": "https://other.company.example/",
	} {
		t.Run("issue "+name, func(t *testing.T) {
			service := New(privateKey, "issuer")
			changed := valid
			changed.Resource = resource
			if _, _, err := service.Issue(changed, "principal", "fingerprint", bundle, now); !errors.Is(err, agentgrant.ErrInvalidTarget) {
				t.Fatalf("issue error = %v, want invalid_target", err)
			}
		})
	}
	...
	wrong := agentgrant.ConsumeRequest{Token: issued.Token, Resource: "https://other.company.example/", Operation: operation}
	if _, _, err := service.Consume(wrong, bundle, now.Add(time.Second)); !errors.Is(err, agentgrant.ErrInvalidTarget) {
		t.Fatalf("wrong-resource consume error = %v, want invalid_target", err)
	}
	correct := agentgrant.ConsumeRequest{Token: issued.Token, Resource: apiResource, Operation: operation}
	if response, _, err := service.Consume(correct, bundle, now.Add(2*time.Second)); err != nil || !response.Consumed {
		t.Fatalf("wrong-resource attempt burned grant: response=%+v err=%v", response, err)
	}
}

...
func NormalizeWithPolicy(input HookInput, subjectID, workloadID string, policy NormalizationPolicy) (authzen.EvaluationRequest, error) {
	...
	if _, exists := input.ToolInput["_bap_agent_grant"]; exists {
		return authzen.EvaluationRequest{}, fmt.Errorf("reserved BAP transport field was supplied by the model")
	}
	...
	httpMethod, bodyDigest, argumentsDigest := "", "", ""
	...
	if action == "mcp.invoke" {
		canonicalArguments, marshalErr := json.Marshal(input.ToolInput)
		if marshalErr != nil {
			return authzen.EvaluationRequest{}, fmt.Errorf("MCP arguments must be JSON-compatible")
		}
		argumentsDigest = "sha256:" + hashID(string(canonicalArguments))
	}
	...
	return authzen.EvaluationRequest{
		...
		Resource: authzen.Entity{Type: "tool-invocation", ID: resourceID, Properties: map[string]any{
			...
			"httpMethod": httpMethod, "bodyDigest": bodyDigest, "argumentsDigest": argumentsDigest,
		}},

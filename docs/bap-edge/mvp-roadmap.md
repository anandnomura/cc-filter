...
| Policy decision | Edge evaluates bundled Cedar locally; an offline activation command signs reviewed source; pilot/production runtime loads only a verified active envelope | File-backed source and active envelope; approval and signing ceremony remain external | Validated immutable lifecycle with staging, rollback, ownership, approval, and fleet rollout | Multi-tenant policy control plane with canary deployment | Pilot signing/runtime separation complete; governed admin lifecycle remains |
